...
def get_json():
    f = open('core/scrape/abstimmungenscrape.json')
    data = json.load(f)
    return data
...
def scrape():
    source = requests.get('https://strafrecht-online.org/archiv/abstimmungen/').text
    ...
    writeToJSONFile('./', 'abstimmungenscrape', articles)
